...
async def ingest_actions_to_graph(
    ...
    existing_user_actions = await graph_service.find_nodes("UserAction", {"engagement_id": engagement_id})
    ...
    if performed_in_rels:
        performed_in_count = await graph_service.batch_create_relationships("PERFORMED_IN", performed_in_rels)
    ...
    preceded_by_rels = []
    for _session_id, session_acts in session_actions.items():
    ...
    preceded_by_count = 0
    if preceded_by_rels:
        preceded_by_count = await graph_service.batch_create_relationships("PRECEDED_BY", preceded_by_rels)
...
async def ingest_case_link_edges(
    ...
    new_case_props_deduped: list[dict[str, Any]] = list({p["case_id"]: p for p in new_case_props}.values())
    if new_case_props_deduped:
        await graph_service.batch_create_nodes("Case", new_case_props_deduped)
        logger.info("Created %d Case nodes for engagement %s", len(new_case_props_deduped), engagement_id)

    # -- Create missing CanonicalEvent nodes ----------------------------------
    new_ce_props: list[dict[str, Any]] = []
    for _link, event in rows:
    ...
    new_ce_props_deduped: list[dict[str, Any]] = list({p["source_event_id"]: p for p in new_ce_props}.values())
    ...
    if case_has_event_rels:
        case_has_event_count = await graph_service.batch_create_relationships("CASE_HAS_EVENT", case_has_event_rels)
...
async def ingest_switching_traces(
    ...
    existing_trace_nodes = await graph_service.find_nodes("SwitchingTrace", {"engagement_id": engagement_id})
    ...
    existing_app_nodes = await graph_service.find_nodes("Application", {"engagement_id": engagement_id})
    ...
    observed_in_count = 0
    if observed_in_rels:
        observed_in_count = await graph_service.batch_create_relationships("OBSERVED_IN", observed_in_rels)
    ...
    involves_count = 0
    if involves_rels:
        involves_count = await graph_service.batch_create_relationships("INVOLVES", involves_rels)
