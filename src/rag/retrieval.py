...
# Stopwords for graph expansion query-term extraction.
# Kept at module level to avoid re-allocating on every call.
_GRAPH_EXPAND_STOPWORDS: frozenset[str] = frozenset(
    {
        "the",
        "a",
        "an",
...
class HybridRetriever:
    ...
    @pipeline_stage("serve", engagement_id_param="engagement_id")
    async def retrieve(
        self,
        query: str,
    ...
    async def _graph_expand(
        ...
        query_terms = [w.lower() for w in query.split() if len(w) >= 3 and w.lower() not in _GRAPH_EXPAND_STOPWORDS]
