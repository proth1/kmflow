...
async def test_scenario_2_ranked_gap_list() -> None:
    ...
    actions = [
        EpistemicActionItem(
            target_element_id="e1",
            target_element_name="A",
            evidence_gap_description="Gap A",
            current_confidence=0.10,
            estimated_confidence_uplift=0.30,
            projected_confidence=0.40,
            information_gain_score=0.25,
            recommended_evidence_category="documents",
            priority="high",
        ),
        EpistemicActionItem(
            ...
            evidence_gap_description="Gap B",
            current_confidence=0.40,
            estimated_confidence_uplift=0.15,
            projected_confidence=0.55,
            information_gain_score=0.10,
            recommended_evidence_category="documents",
            priority="medium",
        ),
        EpistemicActionItem(
            target_element_id="e3",
            target_element_name="C",
            evidence_gap_description="Gap C",
            current_confidence=0.30,
            estimated_confidence_uplift=0.20,
            projected_confidence=0.50,
    ...
    plan = _make_plan_result(
        [
            EpistemicActionItem(
                target_element_id="e1",
                target_element_name="Wire Transfer",
                evidence_gap_description="No evidence",
                current_confidence=0.15,
                estimated_confidence_uplift=0.08,
                projected_confidence=0.23,
                information_gain_score=0.06,
...
async def test_scenario_not_found_returns_404() -> None:
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        resp = await client.post(f"/api/v1/simulations/scenarios/{uuid.uuid4()}/epistemic-plan")
...
def test_plan_result_aggregation() -> None:
    ...
    actions = [
        ...
        EpistemicActionItem(
            ...
            estimated_confidence_uplift=0.04,
            projected_confidence=0.54,
            information_gain_score=0.02,
            recommended_evidence_category="documents",
            priority="medium",
        ),
    ]
