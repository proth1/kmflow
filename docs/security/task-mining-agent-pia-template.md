...
|-----|-----------------|----|----|---|---|---|---|---|
| R1 | PII captured despite filtering — sensitive personal data (SSN, email, account numbers) present in window titles passes through L2 regex | Medium | High | **High** | Four-layer PII architecture (L1+L2 on-device; L3 ML; L4 human review). L4 quarantine ensures no unreviewed data reaches analytics. Regex patterns updated based on engagement context. | Low | Medium | **Low** |
...
| R5 | Unauthorized access to local buffer — attacker gains access to encrypted buffer on device | Low | Medium | **Medium** | AES-256-GCM encryption. Key in macOS Keychain (inaccessible without device unlock credentials; not exportable). Remote revocation triggers key deletion. | Very Low | Low | **Very Low** |
...
|---------|------------|-----------|
| Four-layer PII architecture | L1 capture prevention (Swift), L2 regex scrubbing (Swift+Python), L3 ML NER (backend), L4 human quarantine review | Whitepaper Sec. 4 |
| AES-256-GCM encryption at rest | Local buffer encrypted; key in Keychain | Whitepaper Sec. 5 |
