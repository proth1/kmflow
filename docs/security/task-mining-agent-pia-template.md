...
| R8 | Device loss or theft — device containing local buffer is lost before upload | Low | Medium | **Low** | Local buffer is currently plaintext SQLite (AES-256-GCM encryption planned). Remote revocation via MDM + KMFlow console. 100 MB FIFO cap limits maximum exposure (typically 1–3 days of data). L2 PII scrubbing reduces sensitivity of stored data. | Low | Low | **Low** |
